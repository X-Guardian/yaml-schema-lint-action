...
A composite GitHub Action that lints YAML files against JSON schemas and creates a **GitHub Check** with inline annotations and a markdown summary — similar to ESLint report actions.
...
| `version`                         | yaml-schema-lint version to install (ignored when `binary` is set) | No | `0.2.0-beta.2`     |
...
2. **Install and run** `yaml-schema-lint` via `npx`, producing a JSON report
